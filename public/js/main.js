...
angular.module ('lisk_explorer.currency').controller ('CurrencyController',
  function ($scope, $rootScope) {
    $rootScope.currency.symbol = localStorage && localStorage.getItem ('oxy_explorer-currency') || 'OXY';
...
ActivityGraph.prototype.amount = function (tx, sign) {
    return (sign + tx.amount / Math.pow(10, 8)) + ' OXY';
};
...
var Header = function ($rootScope) {
    ...
    this.updatePriceTicker = function (res) {
        ...
        // When ticker for user-stored currency is not available - switch to OXY temporarly
        if ($rootScope.currency.symbol !== 'OXY' && (!$rootScope.currency.tickers || !$rootScope.currency.tickers.OXY || !$rootScope.currency.tickers.OXY[$rootScope.currency.symbol])) {
            console.log ('Currency ' + $rootScope.currency.symbol + ' not available, fallback to OXY');
            $rootScope.currency.symbol = 'OXY';
        }
    };
...
angular.module('lisk_explorer')
  ...
  .filter('currency', function (numberFilter, liskFilter) {
      return function (amount, currency, decimal_places) {
        ...
        if (currency.tickers && currency.tickers.OXY && currency.tickers.OXY[currency.symbol]) {
          factor = currency.tickers.OXY[currency.symbol];
        } else if (currency.symbol !== 'OXY') {
  ...
  .filter('nethash', function () {
      return function (nethash) {
          if (nethash === '0daee950841005a3f56f6588b4b084695f0d74aaa38b21edab73446064638552') {
              return 'Testnet';
          } else if (nethash === '4c1170a3edb03f961e5e3f7cedcd25563f0a46ec4aa3342715d09c47b398ea19')  {
              return 'Mainnet';
          } else {
              return 'Local';
          }
